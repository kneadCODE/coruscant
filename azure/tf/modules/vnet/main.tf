...
resource "azurerm_virtual_network" "vnet" {
  name                = "vnet-${var.purpose}-${var.env}-${local.region_shorthand[var.region]}-0${var.instance_number}"
...
resource "azurerm_route_table" "route_table" {
  ...
  name                          = "rt-${var.purpose}-${each.key}-${var.env}-${local.region_shorthand[var.region]}-0${var.instance_number}"
  resource_group_name           = var.rg_name
  location                      = var.region
...
resource "azurerm_network_security_group" "subnet" {
  ...
  name                = "nsg-${var.purpose}-${each.key}-${var.env}-${local.region_shorthand[var.region]}-0${var.instance_number}"
  resource_group_name = var.rg_name
  location            = var.region
  tags                = var.tags
}
