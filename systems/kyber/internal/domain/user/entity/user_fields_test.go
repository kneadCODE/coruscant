...
func TestUserID_NewUserIDFromString(t *testing.T) {
	validID, err := NewUserID()
	require.NoError(t, err)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			id, err := NewUserIDFromString(tt.idStr)
...
func TestUserID_Equals(t *testing.T) {
	...
	id1Copy, err := NewUserIDFromString(id1.String())
	require.NoError(t, err)
...
func TestUserID_TypeSafety(t *testing.T) {
	userID, err := NewUserID()
	require.NoError(t, err)

	// This ensures UserID is a distinct type from EntityID
