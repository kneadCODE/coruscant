...
func TestTransactionID_NewTransactionID(t *testing.T) {
...
func TestTransactionID_NewTransactionIDFromString(t *testing.T) {
	validID, err := NewTransactionID()
	require.NoError(t, err)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			id, err := NewTransactionIDFromString(tt.idStr)
...
func TestTransactionID_Equals(t *testing.T) {
	...
	id1Copy, err := NewTransactionIDFromString(id1.String())
	require.NoError(t, err)
...
func TestTransactionID_TypeSafety(t *testing.T) {
	transactionID, err := NewTransactionID()
	require.NoError(t, err)

	// This ensures TransactionID is a distinct type from EntityID
	// and can't be accidentally used interchangeably
	assert.IsType(t, TransactionID{}, transactionID)
}